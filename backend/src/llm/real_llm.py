...
class RealLLM:
    ...
    def _annotate_response(
        ...
        *,
        ...
        """Attach response-origin metadata while preserving the existing response schema."""
        annotated = dict(result)
        annotated["_response_origin"] = origin
        annotated["_fallback_reason"] = fallback_reason
        annotated["_response_provider"] = provider
        return annotated
    ...
    def invoke(self, prompt: dict[str, Any]) -> dict[str, Any]:
        ...
        return self._get_default_response(prompt_type, reason=str(last_error or ""))
    ...
    def _try_backup_api(
        ...
        *,
        ...
        try:
            ...
            default_response = self._get_default_response(
                prompt_type,
                reason=str(exc or fallback_reason),
            )
            self._write_trace_json(trace_dir, "default_response.json", default_response)
            return default_response
    ...
    def _get_default_response(self, prompt_type: str, *, reason: str = "") -> dict[str, Any]:
