...
class Controller:
    ...
    def __init__(
        ...
        base_dir: str | Path,
        llm_service=None,
        global_store: GlobalEventStore | None = None,
    ) -> None:
        """创建 Controller。
        ...
        self.global_store = global_store or GlobalEventStore(self.base_dir / "global_events.db")
    ...
    async def _record_agent_observation(self, agent_id: str, event: EventBase) -> None:
        ...
        await asyncio.to_thread(agent.memory_store.record_event, event)
    ...
    def _export_reflections_markdown(
        ...
        for entry in reflection_entries:
            ...
            if status == "recorded":
                ...
                reflection_source = str(entry.get("source", "")).strip()
                fallback_reason = str(entry.get("fallback_reason", "")).strip()
                if reflection_source:
                    lines.append(f"- Reflection Source: {reflection_source}")
                if fallback_reason:
    ...
    async def _run_reflection_with_timeout(
        self,
        agent: BaseAgent,
        game_id: str,
        ...
        *,
        ...
        """Run one blocking reflection in a daemon thread so a stuck SDK call cannot block process exit."""
        loop = asyncio.get_running_loop()
        future = loop.create_future()

        def worker() -> None:
            try:
                artifact = agent.reflect(game_id, revealed_truth, outcome)
            except Exception as exc:  # pragma: no cover - defensive bridge to asyncio future
                if not future.done():
                    loop.call_soon_threadsafe(future.set_exception, exc)
                return
            if not future.done():
                loop.call_soon_threadsafe(future.set_result, artifact)

        thread = threading.Thread(
            target=worker,
            name=f"reflection-{game_id}-{agent.agent_id}",
            daemon=True,
        )
        thread.start()
    ...
    async def end_game_async(self, game_id: str) -> None:
        ...
        per_agent_timeout = max(int(AppConfig.POST_GAME_PER_AGENT_TIMEOUT_SECONDS), 1)
        reflection_deadline = (
        ...
        for agent_id, agent in self._agents.items():
            ...
            reflection_timeout = max(1.0, min(float(remaining_budget), float(per_agent_timeout)))
            try:
                artifact = await self._run_reflection_with_timeout(
                    agent,
                    game_id,
                    revealed_truth,
                ...
                await self._publish_event(
                    SystemEvent(
                        ...
                        payload={
                            "agent_id": agent_id,
                            "reason": "post_game_reflection_timeout",
                            "timeout_seconds": reflection_timeout,
                        },
                        system_name="reflection_skipped",
                    )
