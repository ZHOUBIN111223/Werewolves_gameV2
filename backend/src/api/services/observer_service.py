"""Observer-facing read models built on top of the event store."""
...
class MatchNotFoundError(Exception):
    """Raised when the requested match does not exist in storage."""
...
class ObserverService:
    ...
    async def get_match_snapshot(self, match_id: str, recent_limit: int = 20) -> MatchSnapshotResponse:
        events = await self._load_match_events(match_id)
    ...
    async def get_timeline(
        ...
        *,
        ...
        await self._ensure_match_exists(match_id)
        events = await self._store.query_game_events(
            match_id,
            event_type=event_type,
            phase=phase,
            actor=actor,
            system_name=system_name,
            visibility_scope=visible_to,
        )
    ...
    async def get_events_after(
        ...
        *,
        ...
        await self._ensure_match_exists(match_id)
        events = await self._store.get_events_after_timestamp(
            match_id,
            after_seq,
            limit=limit + 1,
            event_type=event_type,
    ...
    async def get_match_stats(
        ...
        *,
        ...
        await self._ensure_match_exists(match_id)
        stats = await self._store.get_game_statistics(match_id, visibility_scope=visible_to)
    ...
    async def _load_match_events(self, match_id: str) -> list[EventBase]:
        events = await self._store.get_events_by_game_id(match_id)
        if not events:
            raise MatchNotFoundError(match_id)
        return events

    async def _ensure_match_exists(self, match_id: str) -> None:
        if not await self._store.has_game(match_id):
            raise MatchNotFoundError(match_id)
    ...
    def _build_match_dto(self, match_id: str, events: Iterable[EventBase]) -> MatchDTO:
        ...
        game_ended = any(self._is_system(event, "game_ended") for event in event_list)
        winner = self._find_winner(event_list)
        current_speaker = self._find_last_payload_value(event_list, "speaker")
        focus_target = self._find_focus_target(event_list)
        current_subphase = self._find_last_payload_value(event_list, "subphase")
        alive_players = self._find_last_alive_players(event_list)
        total_players = len(self._find_player_order(event_list))
    ...
    def _find_last_alive_players(self, events: Iterable[EventBase]) -> list[str]:
        event_list = list(events)
        alive = self._find_player_order(event_list)
    ...
    def _is_system(self, event: EventBase, system_name: str) -> bool:
        return (
            ...
            and getattr(event, "system_name", None) == system_name
        )
