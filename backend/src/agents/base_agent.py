...
class BaseAgent(ABC):
    ...
    def remember_fact(self, game_id: str, phase: str | GamePhase, content: str) -> None:
        """Store a compact factual note in the current game's summary memory."""
        phase_str = phase.value if isinstance(phase, GamePhase) else phase
        self.memory_store.append_summary_note(
            game_id=game_id,
            phase=phase_str,
            content=content,
            note_type="fact",
    ...
    def remember_hypothesis(
        ...
        """Store a compact hypothesis note in the current game's summary memory."""
        phase_str = phase.value if isinstance(phase, GamePhase) else phase
        self.memory_store.append_summary_note(
            game_id=game_id,
            phase=phase_str,
            content=f"{content} (confidence={confidence:.2f})",
    ...
    def decide_action(
        ...
        visible_events = self._select_action_visible_events(self.agent_store.read_all())
        request_context = self._extract_request_context(visible_events)
        short_memories = self.memory_store.retrieve_summary_memories(
            game_id=game_id,
            limit=10,
        )
        speech_memories = self.memory_store.retrieve_recent_speeches(
            game_id=game_id,
            limit=8,
        )
        ...
        result = self._normalize_result_for_request(
            result,
    ...
    def _select_action_visible_events(
        self,
        visible_events: list[EventBase],
        limit: int = 25,
    ) -> list[EventBase]:
        """Keep action prompts bounded to a recent event window."""
        if limit <= 0:
            return []
        return visible_events[-limit:]

    def _normalize_result_for_request(
        self,
        ...
        *,
        ...
        """Coerce raw model output into the current request's legal shape before validation."""
        normalized = deepcopy(result)
        request_kind = str(request_context.get("request_kind", "") or "").strip()
        available_targets = [
            str(item)
            for item in request_context.get("available_targets", [])
            if item
        ]
        legal_targets = available_targets or [
            player_id for player_id in alive_players if player_id != self.agent_id
        ]

        def choose_target() -> str:
            target = str(normalized.get("target", "") or "").strip()
            if target and target in legal_targets and target != self.agent_id:
                return target
            return legal_targets[0] if legal_targets else ""

        if request_kind in {"day_speak", "sheriff_campaign_speak", "last_words"}:
            normalized["action_type"] = "speak"
            normalized["target"] = ""
            speech = str(normalized.get("public_speech", "") or "").strip()
            reasoning = str(normalized.get("reasoning_summary", "") or "").strip()
            if not speech:
                normalized["public_speech"] = speech or reasoning or "我先说明当前判断。"
            return normalized

        if request_kind in {"day_vote", "sheriff_vote"}:
            normalized["action_type"] = "vote"
            normalized["target"] = choose_target()
            normalized["public_speech"] = ""
            return normalized

        if request_kind == "badge_transfer":
            action_type = str(normalized.get("action_type", "") or "").strip()
            if action_type not in {"vote", "skip"}:
                action_type = "vote" if legal_targets else "skip"
            normalized["action_type"] = action_type
            normalized["target"] = choose_target() if action_type == "vote" else ""
            normalized["public_speech"] = ""
            return normalized
        ...
        if request_kind == "night_action":
            allowed_actions = [
                str(item)
                for item in request_context.get("available_actions", [])
                if item
            ]
            if len(allowed_actions) == 1:
                normalized["action_type"] = allowed_actions[0]
            action_type = str(normalized.get("action_type", "") or "").strip()
            if action_type in {"kill", "inspect", "protect", "poison", "heal", "vote"}:
                normalized["target"] = choose_target()
    ...
    def _select_reflection_memories(self, game_id: str) -> list[MemoryItem]:
        """Limit reflection memories to compact current-game summaries plus strategy rules."""
        ...
        return self.memory_store.retrieve_reflection_memories(
            game_id=game_id,
            role=self.role,
            limit_summary_items=max_game_memories,
            limit_strategy_rules=max_strategy_rules,
        )
    ...
    def reflect(
        ...
        artifact = ReflectionArtifact(
            ...
            source=str(result.get("_response_origin", "model") or "model"),
            fallback_reason=str(result.get("_fallback_reason", "") or ""),
        )
        return artifact
